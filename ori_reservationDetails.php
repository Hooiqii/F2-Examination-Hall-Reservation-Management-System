    ...
    if (!empty($participantNo) && is_numeric($participantNo)) {
        ...
        if ($endDate > $startDate) {
            ...
            for ($i = 0; $i < $diffInDays; $i++) {
                ...
                WHERE bk_floorSelection = ? AND bk_startDate <= ? AND bk_endDate >= ? AND ((bk_startTime < ? AND bk_endTime > ?) 
                OR (bk_startTime BETWEEN ? AND ?) OR (bk_endTime BETWEEN ? AND ?)) AND (bk_status='Approve')";
                $stmt_total_capacity = $conn->prepare($sql_total_capacity);
                $currentDateStr = $currentDate->format('Y-m-d');
                $stmt_total_capacity->bind_param("sssssssss", $floorSelection, $currentDateStr, $currentDateStr, $endTime, $startTime, $startTime, $endTime, $startTime, $endTime);
            ...
            $sql_total_capacity = "SELECT SUM(bk_participantNo) as total_capacity FROM booking 
            WHERE bk_floorSelection = ? AND bk_startDate <= ? AND bk_endDate >= ? AND ((bk_startTime < ? AND bk_endTime > ?) OR (bk_startTime BETWEEN ? AND ?) OR (bk_endTime BETWEEN ? AND ?)) AND (bk_status='Approve')";;
            $stmt_total_capacity = $conn->prepare($sql_total_capacity);
            $stmt_total_capacity->bind_param("sssssssss", $floorSelection, $endDate, $startDate, $endTime, $startTime, $startTime, $endTime, $startTime, $endTime);
            $stmt_total_capacity->execute();
            $result_total_capacity = $stmt_total_capacity->get_result();
            $data_total_capacity = $result_total_capacity->fetch_assoc();
            $total_capacity_booked = $data_total_capacity['total_capacity'];
